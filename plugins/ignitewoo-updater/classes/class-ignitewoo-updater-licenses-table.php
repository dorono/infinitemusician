...
class IgniteWoo_Updater_Licenses_Table extends WP_List_Table {
	public $per_page = 100;
	public $data, $found_data;
	/**
	 * Constructor.
	 * @since  1.0.0
	 */
	...
	public function no_items () {
		echo wpautop( __( 'No IgniteWoo products found.', 'ignitewoo-updater' ) );
	} // End no_items(0)
	...
	public function get_columns () {
		$columns = array(
		'product_name' => __( 'Product', 'ignitewoo-updater' ),
		'product_version' => __( 'Version', 'ignitewoo-updater' ),
		'product_status' => __( 'License Key', 'ignitewoo-updater' )
		);
		return $columns;
    } // End get_columns()
	...
	public function column_product_status ( $item ) {
		...
		if ( 'active' == $item['product_status'] ) {
			$deactivate_url = wp_nonce_url( add_query_arg( 'action', 'deactivate-product', add_query_arg( 'filepath', $item['product_file_path'], add_query_arg( 'page', 'ignitewoo-licenses', admin_url( 'index.php' ) ) ) ), 'bulk-licenses' );
			$response = '<a href="' . esc_url( $deactivate_url ) . '">' . __( 'Deactivate', 'ignitewoo-updater' ) . '</a>' . "\n";
		} else {
			$response .= '<input name="license_keys[' . esc_attr( $item['product_file_path'] ) . ']" id="license_keys-' . esc_attr( $item['product_file_path'] ) . '" type="text" value="" size="40" aria-required="true" placeholder="' . esc_attr( sprintf( __( 'Place %s license key here', 'ignitewoo-updater' ), $item['product_name'] ) ) . '" />' . "\n";
		}
