...
class IGN_Meta_Box_Coupon_Data {
	...
	public static function output( $post ) {
		...
		<style type="text/css">
			...
			<?php
				...
				// Apply before tax
				// This setting is no longer applicable in WC 2.3 or newer
				...
				?>
					...
					<?php echo '</div><div class="options_group">'; ?>
			...
			<?php if ( version_compare( WOOCOMMERCE_VERSION, '2.3', '>=' ) ) { ?>
				<script>
				jQuery( '#ajax_chosen_select_products_and_variations' ).select2();
				</script>
			<?php } ?>
	...
	public static function save( $post_id ) {
		...
		if ( version_compare( WOOCOMMERCE_VERSION, '2.1', '>=' ) ) {
			IGN_Meta_Box_Coupon_Data::save21( $post_id );
			return;	
		}
	...
	public static function save21( $post_id ) {
		global $wpdb;
		...
		if ( isset( $_POST['exclude_product_ids'] ) ) {
			$exclude_product_ids = explode( ',', $_POST['exclude_product_ids'] );
